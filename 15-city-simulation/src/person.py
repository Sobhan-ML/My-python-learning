...
class Consts:
    ...
    BASE_INCOME = {'worker': {'mine': 800}, 'teacher': {
        'school': 300}, 'engineer': {'company': 1000}}
    MIN_AGE = 15
    AGE_MUL = 10
...
class Person:
    ...
    def do_level(self, income: int) -> float:
        ...
        adjusted_income = income * \
            math.sqrt((self.level * self.work_place.level))
        return float(adjusted_income)
    ...
    def calc(self) -> float:
        ...
        costs = self.calc_life_cost()
        adjusted_income = self.do_level(base_income)
        return adjusted_income - costs
